...
let write_settings = fun airframe_xml xml_file out_set telemetry_xml ->
  ...
  if GC.get_telemetry_of_airframe airframe_xml <> "none" && List.length filtered_xml > 0 then begin
...
let _ =
  ...
  write_settings (ExtXml.parse_file Sys.argv.(1)) Sys.argv.(3) out_set telemetry_xml;
  close_out out_set;
